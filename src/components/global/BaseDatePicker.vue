...
const props = withDefaults(defineProps<{ modelValue: string; label: string; panelAlign?: 'start' | 'end'; minDate?: string; inline?: boolean }>(), { panelAlign: 'start', minDate: '', inline: false })
...
<template>
  <div ref="root" class="date-picker" :class="{ 'date-picker--open': open, 'date-picker--inline': inline }" @keydown="onKeydown">
    <span>{{ label }}</span>
    ...
    <Transition name="date-picker-drop"><div v-if="open" class="date-picker__panel" :class="[`date-picker__panel--${panelAlign}`, { 'date-picker__panel--inline': inline }]">
...
/* Modo inline: el calendario se abre EN FLUJO (empuja el contenido) en vez de flotar y tapar. */
.date-picker--inline .date-picker__panel--inline { position:static; width:100%; max-width:none; margin-top:.5rem; box-shadow:none; }
.date-picker--inline.date-picker--open { z-index:auto; }
</style>
