...
.products-toolbar { align-items: stretch; display: flex; flex-direction: column; gap: .85rem; overflow: visible; padding: 1.15rem; position: relative; z-index: 1; }
.products-toolbar:has(.dropdown.open) { z-index: 200; }
...
.dropdown__menu { background: #fff; border: 1px solid rgba(8,17,13,.12); border-radius: 14px; box-shadow: 0 16px 38px rgba(8,17,13,.16); left: 0; list-style: none; margin: .35rem 0 0; max-height: 280px; overflow-y: auto; padding: .35rem; position: absolute; right: 0; top: calc(100% + .35rem); z-index: 1000; }
