...
<template>
  <section class="products-toolbar panel">
    <label><span>Buscar producto</span><div class="input"><i class="fa-solid fa-magnifying-glass" /><input v-model="searchQuery" placeholder="Código, nombre o descripción" /></div></label>
    <label><span>Categoría</span><BaseSelect v-model="selectedCategory" :options="categoryOptions" placeholder="Todas" /></label>
...
label { display: flex; flex-direction: column; gap: .45rem; } label > span { color: rgba($text-dark,.68); font-size: .82rem; font-weight: 700; letter-spacing: .08em; text-transform: uppercase; }
...
@media (min-width: 769px) { .products-toolbar { align-items: flex-end; flex-direction: row; } label:first-child { flex: 1.5 1 0; } label:nth-child(2) { flex: 1 1 0; } .results, button { flex: 0 0 auto; } }
</style>
