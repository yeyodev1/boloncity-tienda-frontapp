...
<template>
  <div class="info-grid">
    <section class="info-card"><span><i class="fa-solid fa-receipt" /> Costos</span><div><p>Subtotal <strong>${{ (order.subtotal / 100).toFixed(2) }}</strong></p><p v-if="order.deliveryType === 'delivery'">Envío <strong>${{ ((order.deliveryCost || 0) / 100).toFixed(2) }}</strong></p><p class="total">Total <strong>${{ (order.total / 100).toFixed(2) }}</strong></p></div></section>
