...
async function changeStatus(order: OrderDTO, status: OrderStatus) { await move(order, status) }
async function drop(orderId: string, status: OrderStatus) { const order = findOrder(orderId); if (order && order.status !== status) await move(order, status) }
